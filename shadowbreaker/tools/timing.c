/**
 * @file timing.c
 * @brief A program to demonstrate difference between
 *        wall time and clock time.
 */
...
// make argc global - bit of a cheat.
...
int main(int argc, __attribute__((unused)) char **argv) {
  ...
  printf("single threaded cpu work: %.2f seconds wall time, %.2f seconds CPU time\n",
         elapsed_wall, elapsed_cpu);


  // start 4 threads to do some work. Results will differ with # of cores. 
  // Assume 4 cores. This means we should see:
  ...
  elapsed_wall = getTime() - start_wall_time;
  elapsed_cpu  = getCPUTime() - start_cpu_time;
  printf("multi threaded cpu work: %.2f seconds wall time, %.2f seconds CPU time\n",
         elapsed_wall, elapsed_cpu);
